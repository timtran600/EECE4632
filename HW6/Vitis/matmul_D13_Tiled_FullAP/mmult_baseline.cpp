...
#pragma HLS ARRAY_PARTITION variable=tA  dim=2
#pragma HLS ARRAY_PARTITION variable=tB  dim=1
#pragma HLS ARRAY_PARTITION variable=tC  dim=2
        ...
        tile_k: for (int tk = 0; tk < DATA_SIZE/TILE; tk++) {
            ...
            comp_i: for (int i = 0; i < TILE; i++) {
             comp_j: for (int j = 0; j < TILE; j++) {
                 int acc = tC[i][j];       // read once before pipeline
                     comp_k: for (int k = 0; k < TILE; k++) {
                 #pragma HLS PIPELINE II=1
                       acc += tA[i][k] * tB[k][j];
              }
                  tC[i][j] = acc;           // write once after pipeline
                }
            }   
